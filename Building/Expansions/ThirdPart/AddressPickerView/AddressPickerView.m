...
#define VIEWSIZE 130    //150
#define temp ([UIScreen mainScreen].bounds.size)
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (frame.size.height == 0 && frame.size.width == 0) {
        frame = CGRectMake(0, 0, AD_SCREEN.width, AD_SCREEN.height-VIEWSIZE);
    }
...
- (UIButton *)backgroundBtn {
    if (!_backgroundBtn) {
        _backgroundBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, AD_SCREEN.width, AD_SCREEN.height-VIEWSIZE)];
...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        _titleBackgroundView = [[UIView alloc]initWithFrame:
                                CGRectMake(0, AD_SCREEN.height-VIEWSIZE, SELFSIZE.width, TITLEHEIGHT)];
...
- (void)loadAddressData{
    ...
    _citysDict=[[NSDictionary alloc]init];
    NSMutableDictionary *cityMult=[[NSMutableDictionary alloc] init];
    _areasDict=[[NSDictionary alloc]init];
    //NSMutableDictionary *conutryMult=[[NSMutableDictionary alloc] init];
    ...
    [FangYuanNetworkService getHomeCityListSuccess:^(NSArray *array) {
        ...
        for (FYProvinceModel *address in weakSelf.PArray) {
            [weakSelf.provincesArr addObject:address.provinceName];
            NSString *provinceStr=address.provinceName;
           
            NSMutableArray *cityTemps=[NSMutableArray array];
            for (FYCityModel *city in address.cityList) {
               [cityTemps addObject:city.cityName];
//               NSMutableArray *countryTemps=[NSMutableArray array];
               NSString *cityName=city.cityName;
//               NSString *proCityName=[NSString stringWithFormat:@"%@-%@",provinceStr,cityName ];
//                for (FYCountryModel *countryModel in city.countryInfoList) {
//                     [countryTemps addObject:countryModel.countryName];
//                }
//                //weakSelf.areasDict=@{cityName:countryTemps};
//                 NSDictionary *CountryDict=@{proCityName:countryTemps};
//                 [conutryMult addEntriesFromDictionary:CountryDict];
//
            }
            //weakSelf.citysDict=@{provinceStr:cityTemps};
            NSDictionary *dict=@{provinceStr:cityTemps};
            //[mutDic2 addEntriesFromDictionary:dic5];
            [cityMult addEntriesFromDictionary:dict];
            //NSLog(@"citysDict:%@",weakSelf.citysDict);
//             [weakSelf.citysDict setObject:@cityTemps forKey:@cityStr];
           //NSLog(@"address:%@",address.provinceName);
        
          weakSelf.citysDict=cityMult;
//        weakSelf.areasDict=conutryMult;
//         NSLog(@"citysDict:%@",weakSelf.citysDict);
//         NSLog(@"areasDict:%@",weakSelf.areasDict);
//        "河北省": ["石家庄市", "唐山市", "秦皇岛市", "邯郸市", "邢台市", "保定市", "张家口市", "承德市", "沧州市", "廊坊市", "衡水市"],
//
       // NSLog(@"prvincessArr:%@",weakSelf.provincesArr.copy);
       //NSLog(@"prvincessArr:%@",weakSelf.provincesArr[1]);
//        weakSelf.citysDict    = [weakSelf.dataDict objectForKey:@"cityList"];
//          NSLog(@"cityList:%@",weakSelf.citysDict);
...
//
//        weakSelf.areasDict    = [weakSelf.dataDict objectForKey:@"countryInfoList"];
...
- (void)showOrHide:(BOOL)isShow animation:(BOOL)animation{
    ...
    if (!animation) {
        if (isShow) {
            ...
            selfY = AD_SCREEN.height-VIEWSIZE - CONTENTHEIGHT - TITLEHEIGHT;
            if (AD_iPhoneX) {
                selfY -= 34.0f;
            }
        }
            ...
            selfY = AD_SCREEN.height-VIEWSIZE;
        }
        self.titleBackgroundView.frame = CGRectMake(0,selfY, self.bounds.size.width,TITLEHEIGHT);
        //NSLog(@"titleBackgroundView !animation x:%lf, y:%lf, w:%lf, h:%lf", self.titleBackgroundView.frame.origin.x, self.titleBackgroundView.frame.origin.y, self.titleBackgroundView.frame.size.width, self.titleBackgroundView.frame.size.height );
        ...
        //NSLog(@"addressPickerView !animation x:%lf, y:%lf, w:%lf, h:%lf", self.addressPickerView.frame.origin.x, self.addressPickerView.frame.origin.y, self.addressPickerView.frame.size.width, self.addressPickerView.frame.size.height );
        if (AD_iPhoneX) {
            self.maskingView.frame = CGRectMake(0, self.addressPickerView.frame.origin.y + self.addressPickerView.frame.size.height, AD_SCREEN.width, 34);
        }
        return;
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        if (isShow) {
            self.backgroundBtn.hidden = NO;
            selfkY = AD_SCREEN.height-VIEWSIZE - CONTENTHEIGHT;
            //NSLog(@"AD_SCREEN.height:%lf, selfkY:%lf", AD_SCREEN.height, selfkY );
            ...
            selfkY = AD_SCREEN.height-VIEWSIZE;
            //NSLog(@"AD_SCREEN.height:%lf, selfkY:%lf", AD_SCREEN.height, selfkY );
        }
        self.titleBackgroundView.frame = CGRectMake(0,selfkY, self.bounds.size.width,TITLEHEIGHT);
    //NSLog(@"titleBackgroundView x:%lf, y:%lf, w:%lf, h:%lf", self.titleBackgroundView.frame.origin.x, self.titleBackgroundView.frame.origin.y, self.titleBackgroundView.frame.size.width, self.titleBackgroundView.frame.size.height );
        
        self.addressPickerView.frame = CGRectMake(0, self.titleBackgroundView.frame.origin.y + TITLEHEIGHT, AD_SCREEN.width, CONTENTHEIGHT - TITLEHEIGHT);
        //NSLog(@"addressPickerView x:%lf, y:%lf, w:%lf, h:%lf", self.addressPickerView.frame.origin.x, self.addressPickerView.frame.origin.y, self.addressPickerView.frame.size.width, self.addressPickerView.frame.size.height );
        if (AD_iPhoneX) {
            self.maskingView.frame = CGRectMake(0, self.addressPickerView.frame.origin.y + self.addressPickerView.frame.size.height, AD_SCREEN.width, 34);
        }
...
//        }
        ...
        if (self.isAutoOpenLast) {
            [self setProvinceIdx:selectProvince cityIdx:selectCity areaIdx:nil];
        }
        ...
        self.proName=p.name;
        self.cityName=c.cityName;
       // self.areaName=c.areas[selectArea];
    }
}
