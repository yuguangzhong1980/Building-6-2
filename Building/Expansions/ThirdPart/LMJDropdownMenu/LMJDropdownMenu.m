...
- (void)createMainBtnWithFrame:(CGRect)frame{
    ...
    [_mainBtn.layer setBorderColor:color];
    _mainBtn.layer.cornerRadius=4;
    ...
    _arrowMark.image  = [UIImage imageNamed:@"list_cbb_n"];
...
- (void)showDropDown{   // 显示下拉列表
    ...
    //[self.superview bringSubviewToFront:_listView]; // 将下拉列表置于最上层
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _listView.frame  = CGRectMake(VIEW_X(_listView), VIEW_Y(_listView), VIEW_WIDTH(_listView), _rowHeight * 3);
